...
int main(int ac, char **av)
{
	...
	/* used for execve */
	char **command = NULL;
	/* used for allocating space for command array */
	...
	while (1)
	{
		...
		{
			prompt_bytes_written = write(1, prompt, get_strlen(prompt));
		...
		read_line = _getline(&line, &len, stdin);
		/*printf("line is: %s\n", line);*/
		if (read_line == EOF)
		{
			break;
		...
		if (_strmerge(line, "env") == 0)
		{
			...
			for (; environ[i]; i++)
			{
				...
				write(1, environ[i], get_strlen(environ[i]));
				write(1, "\n", 1);
			}
			free(line);
			continue;
		...
		if (path == NULL)
		{
			...
			max_dirs = 100; /*max_path = 4096*/
			directories = malloc(sizeof(char *) * max_dirs);
			directories_i = 0;

			while (directories_token != NULL)
			{
				if (directories_i >= max_dirs)
				{
					max_dirs *= 2;
					directories = realloc(directories, sizeof(char *) * max_dirs);
					if (directories == NULL)
					{
						perror("realloc");
					}
			...
			while (directories[searching_i] != NULL)
			{
				...
				while (entry != NULL)
				{
					if (_strmerge(command[0], entry->d_name) == 0)
					{
						found = 1;
						break;
					}
		...
		else if (child_pid == 0)
		{
			...
			{
				size = get_strlen(directories[searching_i]) + get_strlen(line) + 2;
				...
				strconcat(concat_path, "/");
				strconcat(concat_path, command[0] /*line*/);
		...
		if (path == NULL)
		{
			for (dir_count = 0; directories[dir_count] != NULL; dir_count++)
			{
				free(directories[dir_count]);
			}
		}
